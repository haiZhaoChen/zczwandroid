...
public class DePinyinHelper {
	...
	public DePinyinHelper() {
		...
		try {
			InputStream inputStream = RongContext.getInstance().getResources()
					.getAssets().open(FILE_PATH);
		...
		if (PINYINS != null && input != null) {
			...
			for (int i = 0, len = chars.length; i < len; i++) {
				if (isHanzi(chars[i])) {
					...
					pinyins.add(NONE.equals(pinyin) ? String.valueOf(chars[i])
							: pinyin);
				} else {
					if (i < len - 1 && !isHanzi(chars[i + 1])) {
						pinyin = new String(
								new char[] { chars[i], chars[i + 1] });
						i++;
					} else {
	...
	public String getPinyins(String input, String separator) {
		...
		if (PINYINS != null && input != null) {
			...
			for (int i = 0, len = chars.length; i < len; i++) {
				if (isHanzi(chars[i])) {
					pinyin = PINYINS[chars[i] - First_HANZI];
					pinyins.append(NONE.equals(pinyin) ? String
							.valueOf(chars[i]) : pinyin);
				} else {
		...
		if (PINYINS != null && input != null) {
			...
			for (int i = 0, len = chars.length; i < len; i++) {
				if (isHanzi(chars[i])) {
					pinyin = PINYINS[chars[i] - First_HANZI];
					pinyins.append(NONE.equals(pinyin) ? String
							.valueOf(chars[i]) : pinyin.charAt(0));
				} else {
					pinyins.append(String.valueOf(chars[i]));
				}
				pinyin = null;
