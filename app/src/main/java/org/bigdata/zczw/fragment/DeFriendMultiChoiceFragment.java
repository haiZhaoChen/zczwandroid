...
public class DeFriendMultiChoiceFragment extends DeFriendListFragment implements
		Handler.Callback {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (getActivity().getIntent() == null
				...
				|| !getActivity().getIntent().getData().getScheme()
						.equals("rong")) {
		...
		Log.e("", "----demoacitivity  targetId----:" + ",targetIds----"
				+ targetIds + ",mConversationType--" + mConversationType
				+ ",mDiscussionId---" + mDiscussionId);
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		ImageView tilefinish = (ImageView) view
				.findViewById(R.id.send_message_finish);
	...
	private void selectButtonShowStyle(int selectedCount) {
		if (selectedCount > 0) {
			...
			selectButton.setText(String.format(mConfirmFromatString,
					selectedCount));
	...
	private final void selectPeopleComplete() {
		if (mAdapter == null)
			return;
		ArrayList<UserInfo> userInfos = ((DeFriendMultiChoiceAdapter) mAdapter)
				.getChoiceUserInfos();
		...
		if (mConversationType == Conversation.ConversationType.DISCUSSION
				|| userInfos.size() + mMemberIds.size() > 1) {
			StringBuilder sb = new StringBuilder();

			String userid = null;

				userid = SPUtil.getString(getContext(), "DEMO_USERID", null);
				UserInfo userInfo = Singleton.getInstance().getUserInfoById(getContext(), userid);
				// }
				...
				if (mMemberIds.size() == 0) {
					RongIM.getInstance().createDiscussionChat(getActivity(),
							ids, sb.toString());
					getActivity().finish();
				} else {
					...
					if (!TextUtils.isEmpty(mDiscussionId)) {
						...
						RongIM.getInstance()
								.getRongIMClient()
								.addMemberToDiscussion(mDiscussionId, ids,
			...
			mCallback = new DeFriendMultiChoiceAdapter.MutilChoiceCallback() {
				...
				public void callback(int count) {
					...
					if (!isShow) {
						...
						if (targetIds != null)
							mHandle.obtainMessage(HANDLE_UPDATE_CONFIRM_BUTTON,
									count + targets.length - mMemberIds.size())
									.sendToTarget();
						else
							mHandle.obtainMessage(HANDLE_UPDATE_CONFIRM_BUTTON,
									count - mMemberIds.size()).sendToTarget();
					}
				}
			};
		}
